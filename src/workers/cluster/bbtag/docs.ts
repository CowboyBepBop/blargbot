...
import { SubtagHandlerCallSignature, SubtagParameter } from '@cluster/types';
...
function toField(subtag: BaseSubtag, signature: SubtagHandlerCallSignature, index: number): EmbedFieldData {
    ...
    const defaultDesc = signature.parameters
        .flatMap<SubtagParameter>(p => p.values)
        .filter(param => param.defaultValue !== '')
        .map(param => `\`${param.name}\` defaults to \`${param.defaultValue}\` if not given`)
        .join('\n');
