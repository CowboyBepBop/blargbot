            ...
            definition: [
                {
                    ...
                    */
                    ...
                    execute: (ctx, [channelId, messageId]) => this.removeAllReactions(ctx, channelId.value, messageId.value)
                }
            ]
        });
    }

    public async removeAllReactions(context: BBTagContext, channelStr: string, messageStr: string): Promise<undefined> {
        let message;
        const channel = await context.queryChannel(channelStr, { noLookup: true }) ?? context.channel;
