...
export class UserHasRolesSubtag extends BaseSubtag {
            ...
            definition: [
                {
                    ...
                    execute: (ctx, [roleIds]) => this.userHasRoles(ctx, roleIds.value, '', false)
                },
                {
                    ...
                    execute: (ctx, [roleIds, user, quiet]) => this.userHasRoles(ctx, roleIds.value, user.value, quiet.value !== '')
                }
            ]
        });
    }
    ...
    public async userHasRoles(
        ...
        return result.hasRole.reduce((a, b) => a && b, true);
    }
}
